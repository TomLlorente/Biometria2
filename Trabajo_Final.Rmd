...
**Figura 1. Exploración preliminar de los datos.**\
Los puntos representan las proporciones individuales de frases que evocaron respuesta en relación con el total de frases presentes para cada estímulo (*BOS*, *CON* o *REV*), medidas a una profundidad y con tetrodos determinados.
...
Odds = Probabilidad de frases que evoquen respuesta sobre probabilidad de frases que no evoquen respuesta.
...
efectos_aleatorios <- as.numeric(ranef(m1)$cond$Ave[[1]])

# Realizar la prueba de normalidad Shapiro-Wilk
shapiro_test <- shapiro.test(efectos_aleatorios)
...
alfai1 <- ranef(m1)
# Realizar el QQ plot
qqPlot(alfai1$cond$Ave$`(Intercept)`,
...
Figura 5.
QQ-Plot de los efectos aleatorios del ave para el modelo 1.
No se observan desviaciones significativas de los quantiles predichos.
...
### Ajuste del modelo con una distribución beta-binomial (Modelo con interaccion)
...
```{r}
# Extraer los efectos aleatorios y convertir a vector numérico 
efectos_aleatorios <- as.numeric(ranef(m2)$cond$Ave[[1]])  
# Realizar la prueba de normalidad Shapiro-Wilk 
shapiro_test <- shapiro.test(efectos_aleatorios)  
# Imprimir el resultado de la prueba de Shapiro-Wilk 
print(shapiro_test) 
...
Habiendo validado todos los supuestos, ahora se presentan los resultados obtenidos del modelado:
...
A pesar de que las interacciones no fueron significativas, no las descartaremos del modelo, ya que están determinadas por el diseño experimental.
Además, dado el bajo número de unidades muestrales (N = 5), es posible que la prueba no haya tenido suficiente potencia para detectar la significancia de estas interacciones.
...
Esto sugiere que al menos uno de los estímulos difiere de los demás, por lo que realizamos un análisis post-hoc (Tukey) utilizando la librería `emmeans` para identificar qué estímulo es el que difiere del resto.

### Comparaciones de medias marginales (Tukey) para Estímulo

Realizamos las comparaciones primero en la escala Odds (probabilidad de frases que evoquen respuesta/probabilidad de frases que no evocan respuesta) y luego en escala de la variable respuesta, ambas interpretables.

**Comparaciones en la escala Odds**
...
```{r}
Comp <- emmeans(m2, pairwise ~ Estímulo, adjust = "tukey", type = "link")
summary(Comp)
...
Al revisar los intervalos de confianza de estas comparaciones, podemos interpretar que existe entre un **70% y 170%** más de probabilidad de que una frase evoque una respuesta neuronal en el **BOS** en comparación con el **CON** o el **REV**, con un 95% de confianza.

Esto puede ser observado gráficamente utilizando también la librería `emmeans` (figura 10)

```{r}
# Crear el plot
plot(Comp, comparisons = TRUE, 
     main = "Comparaciones entre Estímulos", 
     xlab = "Probabilidad de Respuesta(escala Odds)", ylab = "Estímulos") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Rotar etiquetas si es necesario
```

**Figura 10. Comparaciones a posteriori de Tukey.** Plot de `emmeans` para observar las diferencias entre los estímulos en la escala de Odds.
...
### Comparaciones en la escala original de la variable de respuesta (Proporción)

```{r}
Comp2 <- emmeans(m2, pairwise ~ Estímulo, type = "response")
Comp2
confint(Comp2)
...
Este patrón se observó tanto en la escala de odds como en la variable de respuesta directa, donde se encontró que el BOS incrementó los odds de respuesta entre un 340% y 1300% respecto de los otros estímulos, reafirmando la sensibilidad del HVC al canto propio.
...
Para aumentar el efecto de los términos del modelo, se podria aumentar el n de unidades experimentales, disminuir el ruido (aumentar las réplicas) o aumentar la magnitud de efecto.
